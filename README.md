...
This is a reactive images processing service that runs on MongoDB and may be used to enpower your apps with the images support.
...
* Java 20
* MongoDB 6
* Redis 7 (optional) ← for the replication purposes
...
```cmd
java -jar -Xmx512M target/images-1.0.0.jar \
    --server.port=8000 \
...
> [!NOTE]
> To scale deployment for the better throughput there's also Redis required. Once it's available restart the deployment with updated Redis settings and `lock.enabled == true`
